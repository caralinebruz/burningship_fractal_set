...
{
    //! [burningship-simd]
    void parallelsimdburningship(int*pixelMatrix, int rows, int cols, const float x1, const float y1, const float scaleX, const float scaleY)
    {
        int NTHREADS = 32;
        int max_iter = 500;
...
int main()
{
    ...
    t.tic();
    parallelsimdburningship(pixelMatrix, rows_x, cols_y, x1, y1, scaleX, scaleY);
    printf("time to compute OMP SIMD version = %f s\n", t.toc());
    ...
    imwrite("burningship_omp_simd.png", burningshipImg);
